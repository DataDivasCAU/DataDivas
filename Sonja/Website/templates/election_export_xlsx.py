...
def _pretty(p: str) -> str:
    m = {"gruene":"Grüne","grüne":"Grüne","linke":"Linke","cdu":"CDU","csu":"CSU","spd":"SPD","fdp":"FDP","afd":"AfD","bsw":"BSW"}
...
def _add_election(rows: List[dict], emap: Dict[str, float]) -> List[dict]:
    ...
    for p in sorted(parties):
        ...
        out.append({"party": p, "posts": float(base.get("posts", 0)), "election": float(emap.get(p, 0))})
    return out

def build_workbook(posts_data: dict, election_data: dict) -> BytesIO:
    """
    posts_data: { "firstElection":[{"party":..., "posts":...}, ...], "secondElection":[...] }
    election_data: { "election2025":[{ "CDU":22.6, ... }], "election2021":[{ ... }] }
    returns BytesIO of XLSX with 3 Sheets + echten Tortendiagrammen
    """
    # Posts
    first_rows  = [{ "party": _pretty(r["party"]),  "posts": float(r.get("posts",0))} for r in posts_data.get("firstElection", [])]
    second_rows = [{ "party": _pretty(r["party"]),  "posts": float(r.get("posts",0))} for r in posts_data.get("secondElection", [])]
    # Election (%)
    e25_obj = (election_data.get("election2025") or [{}])[0]
    e21_obj = (election_data.get("election2021") or [{}])[0]
    e25_map = { _pretty(k): float(v or 0) for k,v in e25_obj.items() }
    e21_map = { _pretty(k): float(v or 0) for k,v in e21_obj.items() }
    ...
    # ===== Sheet 2025 =====
    ws25 = wb.active; ws25.title = "2025"
    ws25.append(["Partei", "Posts", "Election (%)"])
    for c in ws25[1]: c.style = header_style
    for r in first: ws25.append([r["party"], r["posts"], r["election"]/100.0])  # echte Prozentzahl
    ...
    pie25 = PieChart()
    pie25.add_data(Reference(ws25, min_col=3, min_row=1, max_row=ws25.max_row), titles_from_data=True)
    pie25.set_categories(Reference(ws25, min_col=1, min_row=2, max_row=ws25.max_row))
    pie25.title = "Wahlergebnisse 2025"
    ...
    for r in second: ws21.append([r["party"], r["posts"], r["election"]/100.0])
    ...
    pie21 = PieChart()
    pie21.add_data(Reference(ws21, min_col=3, min_row=1, max_row=ws21.max_row), titles_from_data=True)
    pie21.set_categories(Reference(ws21, min_col=1, min_row=2, max_row=ws21.max_row))
    pie21.title = "Wahlergebnisse 2021"
    ws21.add_chart(pie21, "E2")

    # ===== Sheet Both =====
    wsB = wb.create_sheet("Both")
    wsB.append(["Partei", "First Posts", "First Election (%)", "Second Posts", "Second Election (%)"])
    ...
    parties = sorted(set(pf_posts)|set(ps_posts)|set(pf_elec)|set(ps_elec))
    for p in parties:
        wsB.append([p, pf_posts.get(p,0.0), (pf_elec.get(p,0.0))/100.0, ps_posts.get(p,0.0), (ps_elec.get(p,0.0))/100.0])
    ...
    # zwei Pies (2025 & 2021)
    pieB1 = PieChart()
    pieB1.add_data(Reference(wsB, min_col=3, min_row=1, max_row=wsB.max_row), titles_from_data=True)
    pieB1.set_categories(Reference(wsB, min_col=1, min_row=2, max_row=wsB.max_row))
    pieB1.title = "Election 2025"
    wsB.add_chart(pieB1, "G2")
    pieB2 = PieChart()
    pieB2.add_data(Reference(wsB, min_col=5, min_row=1, max_row=wsB.max_row), titles_from_data=True)
    pieB2.set_categories(Reference(wsB, min_col=1, min_row=2, max_row=wsB.max_row))
    pieB2.title = "Election 2021"
    wsB.add_chart(pieB2, "O2")
